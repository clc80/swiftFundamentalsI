...
func HowManyCompromisedAgents() -> Int {
...
let totalNumberOfCompromisedAgents = HowManyCompromisedAgents()
...
func findHighRisk() {
    var nameAndAccessLevel = ""
    
    for agent in agents {
        if agent.accessLevel >= 8 {
            nameAndAccessLevel = "\(agent.realName) access level: \(agent.accessLevel)"
            if agent.compromised {
                print("\(nameAndAccessLevel) **WARNING** **COMPROMISED**")
            } else {
                print(nameAndAccessLevel)
            }
        }
    }
}
...
func classifyAgents() {
    ...
    for agent in agents {
        ...
        case 8...10:
            highLevelAgents += 1
        default:
            print("Agent level does not exist")
        }
    }
...
func sortedAgents() {
    ...
    for agent in sortedAgents {
        print("\(agent.coverName) has access level \(agent.accessLevel)")
    }
}
